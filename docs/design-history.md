...
오늘의 check 어휘는 naming + header_directive(AST 없이 실행 가능한 것)이고, 구조(계층·소유권) check는
tree-sitter가 전제인 다음 증분이다. 이 레지스트리는 **추론이 자꾸 못 재는 check를 제안하는 순간이 다음에 만들
verifier**라, 수요 기반으로 자란다.
