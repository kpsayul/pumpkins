...
(`collect_macros`). 추측 대신 리포에 물어보는 것 — 규칙을 다루는 방식과 같은 태도입니다.

복구는 **오류가 줄어들 때만 채택**됩니다. 이 불변식 덕분에 리포별 허용목록 없이 모든 파일에 그냥
돌릴 수 있습니다. 그리고 지우지 않고 **같은 길이의 공백으로** 바꿉니다 — 줄 번호가 밀리면 구조 지적이
엉뚱한 코드를 가리키니까요.
...
yaml-cpp에서 앞은 25 → 0, 뒤는 337 → 235입니다. 남은 235곳은 진짜 문법 한계인데 클래스는 전부
정상 추출됩니다. 여기에 경고를 걸면 **진지한 C++ 리포마다 항상 켜져 있는 경고**가 되고, 항상 켜진
