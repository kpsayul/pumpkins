...
**먼저: 대개 필요 없습니다.** `kind: "query"` 가 있어서 AI가 검사를 직접 씁니다
([cpp/query.py](../src/pumpkins/languages/cpp/query.py)) — 새 종류의 관행마다 코드를 고칠 일이
원칙적으로 없습니다. 내장 6종은 자주 쓰이는 경우의 **지름길**로 남아 있을 뿐입니다(더 싸고, 검증된
`naming` 규칙은 facet 검사기로 이어집니다).

질의는 두 개가 한 쌍입니다 — `population`(분모: 규칙이 다루는 자리)과 `conforming`(분자: 만족하는 자리),
둘 다 판정 대상을 `@subject` 로 잡습니다. **분모가 필수 칸인 게 핵심**이라, 모델이 "무엇으로 나눌지"를
건너뛸 수 없습니다.

지름길을 새로 만들 가치가 있는지는 learn 출력의 `검사 종류` 집계를 보세요. 특정 종류가 계속 질의로
표현되고 그 질의가 자꾸 틀리면, 그게 내장 검사로 만들 후보입니다.
...
**닫힌 어휘는 [vocabulary.py](../src/pumpkins/languages/cpp/vocabulary.py) 에 `Literal` 로 선언하세요.**
모델이 채우는 필드의 값 집합을 알 수 있다면, 프롬프트에 적지 말고 스키마로 강제해야 합니다 — 프롬프트는
부탁이고 스키마는 계약입니다. `kind`·`facet` 은 처음부터 Literal 이라 한 번도 틀린 값이 온 적이 없고,
자유 문자열이던 `category`·casing 에서만 사고가 났습니다(`UpperCamelCase` 오타 하나로 맞는 규칙이
0/347 로 기각). 반대로 접두사 문자열처럼 **진짜로 열린 값**을 막는 것은 반대 방향의 실수입니다.
