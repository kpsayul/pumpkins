...
| `languages/cpp/ast.py` | C++ AST(tree-sitter) — 식별자·함수·멤버·클래스 추출, 매크로 class 헤더 복구, **파싱 건강상태 측정** (네이티브 dep, import-가드로 격리) | 공통 |
...
- **못 본 것을 말한다** — 안 읽은 파일·분석 못 한 헤더가 있으면 리포트에 이유와 함께 나열하고, 그런 공백이 있으면 finding이 0건이어도 통과(✅)로 쓰지 않는다. **못 읽은 코드도 마찬가지다**: 파서가 판정 못 한 class 선언 수를 세어 남기고(`scan.parse_health`), 통계는 자기를 만든 읽기의 품질을 그대로 물려받는다는 사실을 숨기지 않는다.
- **경고는 항상 의미가 있어야 한다** — 그래서 통계에 영향을 주는 것(판정 못 한 class 선언)에만 경고하고, 영향 없는 것(템플릿 문법 한계)은 숫자로만 남긴다. 항상 켜진 경고는 아무도 읽지 않아 정작 중요한 하나를 묻는다.
