...
`extractor.py`는 이제 **통계만** 담당합니다("이 이름들의 공통점은?"), 문법은 `cpp.py`가 답합니다("선언은 어떻게 생겼나").
...
> 짚었습니다. 그래서 쪼개짐이 감지되면 CLI가 강한 모델로 재실행하라고 안내합니다 — 비용 결정은
> 사용자 것이므로 자동으로 바꾸지는 않습니다.
