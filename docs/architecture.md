...
훑기는 "어떤 파일이 흥미로운가"를 판단하고, 구조 요약은 "어떤 사실이 중요한가"를 판단한다.
**둘 다 도구 작성자의 사전 판단이다.** 전부 읽으면 그 자리가 사라진다 — 어떤 종류의 관행을
찾을지 아무도 미리 정하지 않는다.

```
리포 전체 ──▶ 묶음으로 자르기 ──▶ 묶음마다 [전체 지도 + 그 조각 전문] ──▶ 관찰
             (디렉터리·헤더/소스 짝)      싼 모델                          │
                                                                          ▼
                                              관찰마다 붙은 check 를 리포 전체에 측정
                                                          │
                                                          ▼
                                                   게이트 → 후보
```

- **자르는 기준**은 경로 순서가 아니라 코드의 결이다. 헤더와 짝이 되는 소스는 같은 묶음에,
  파일은 디렉터리 단위로 묶는다. 선언과 구현이 두 호출로 갈리면 어느 쪽도 온전한 것을 못 본다.
  예산보다 큰 파일 하나는 줄 단위로 잘라 **전량을 덮는다** — 앞부분만 보내고 "전부 읽었다"고 하면
  그게 바로 이 도구가 없애려는 조용한 공백이다.
- **조각은 판단하지 않는다.** 열 개 파일만 본 읽기는 그게 리포 전체의 관행인지 알 수 없고, 알
  필요도 없다. 프롬프트가 명시적으로 "확신하지 말고, 약해 보여도 말하라"고 지시한다. 확신을
  요구하면 세 번 본 패턴을 잡음으로 보고 침묵하는데, 열 묶음에서 세 번씩 나온 것이야말로
  게이트가 확인해 줄 관행이다.
- **조각에게 전체 지도를 같이 준다.** 계층 방향처럼 총합에만 존재하는 사실은 어느 조각에도 없다.
  지도는 ~1,100 토큰이라 매 묶음에 붙여도 부담이 없다.
- **싼 모델만 쓴다.** 강한 모델은 관여하지 않는다 — 그게 비용 논거의 전부다. 실측(yaml-cpp):
  97개 파일 전부 읽기가 6개 파일을 강한 모델로 읽는 것과 비슷한 값이다.
- **보내기 전에 규모를 말한다.** 리포 전체를 외부로 보내는 것은 사용자의 결정이고, 파일 수·토큰·
  예상 비용이 눈앞에 있어야 결정할 수 있다.

이 모드가 성립하는 이유는 이 단계가 아니라 **뒤에 있다.** 묶음 열둘에서 나온 관찰 백 개를 *말로*
합치려면 다시 모델을 불러야 하고, 그러면 "몇 곳에서 관찰됐나"라는 유일한 객관적 신호가 모델의
의견이 된다(심판이 선수). 관찰마다 check 가 붙어 있으면 **합칠 필요가 없다** — 각각을 리포 전체에
재보면 같은 것을 재는 관찰끼리 같은 숫자로 수렴하고, 틀린 것은 낮은 coverage 로 기각된다.
그래서 `kind: query` 가 이 모드의 전제 부품이다.
