...
이러면 4.1(추론엔 좋은 모델)과 4.2(대량 입력엔 비용 통제)를 동시에 만족한다. 지금은
쪼개짐이 감지되면 CLI가 "좋은 모델로 다시 돌리라"고 안내만 하는 **잠정 상태**이고,
2단 구조가 들어오면 그 안내는 사라진다.
...
0. **learn 2단 여과 + 구조적 규칙** — 통계로 못 찾는 관행(계층 방향, 소유권, 팩토리 반환 타입)을
   찾으려면 learn이 코드를 읽어야 하고, 그러면 입력이 커져 모델 비용 통제가 필요해진다.
   싼 모델이 1차로 후보를 추리고 좋은 모델이 어려운 것만 판정하는 구조 (§4.3).
