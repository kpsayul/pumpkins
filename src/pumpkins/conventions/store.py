"""Rule persistence — the `conventions/` directory.
    ...
    pumpkins/
    ├── settings.yml       사람이 쓴다 — 확장자 매핑 등. learn이 절대 건드리지 않는다
    ├── learn-report.yml   learn이 쓴다 — 지난 학습이 무엇을 보고 무엇을 버렸는지
    ├── rules/           활성 — 리뷰가 적용하는 것은 여기뿐
    ├── candidates/      사람 판단 대기 — 리뷰에 영향 없음
    └── archive/         기각 또는 은퇴 (reason이 어느 쪽인지 말해준다)

두 파일을 나눈 기준은 **누가 쓰는가**다. `learn-report.yml`은 매 실행 덮어써지는
기록이라(설정이 아니다) 사람이 적은 값이 거기 있으면 사라진다 — 규칙 저장소를
재구조화하며 고친 바로 그 결함이다. 사람이 쓸 값은 `settings.yml`에 둔다.

이름을 `config.yml`에서 바꾼 이유도 같다. 그 파일은 설정처럼 보였지만 실제로는
"지난 학습이 무엇을 측정하고 무엇을 버렸는지"의 기록이고, 이름이 사람에게
고쳐도 되는 것처럼 오해를 줬다.
...
`git log --follow pumpkins/rules/<id>.yml`이 이력이다. 파일 안에 history
...
LEARN_REPORT_FILENAME = "learn-report.yml"  # learn 이 쓴다 (매 실행 덮어씀)
SETTINGS_FILENAME = "settings.yml"  # 사람이 쓴다 (languages/ 가 읽는다)
...
def load_active_rules(path: Path) -> list[ConventionRule]:
    ...
    Accepts either a `pumpkins/` store or a legacy single `conventions.yml`,
...
# 상태를 바꾸려면 파일을 옮기세요:  git mv pumpkins/candidates/{name} pumpkins/rules/
#
...
def write_scan_report(
    ...
    Overwritten every run, which is why anything a human writes by hand belongs
    in settings.yml instead.
    """
    ...
    header = (
        ...
        "# 이 파일은 learn 을 돌릴 때마다 덮어써집니다 — 손으로 적을 값은\n"
        "# settings.yml 에 두세요.\n"
    )
    path = root / LEARN_REPORT_FILENAME
