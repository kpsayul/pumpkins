...
- **검사 어휘는 더 이상 고정 목록이 아닙니다.** 명명·헤더 지시자·계층 방향·반환 타입·소유권·상속은 내장 검사로, 그 밖은 AI가 tree-sitter 질의를 직접 써서 측정합니다(`kind: query`). 다만 실제 실행에서 모델은 아직 내장 지름길을 더 자주 고릅니다 — 얼마나 쓰이는지는 learn 출력의 `검사 종류` 집계로 확인할 수 있습니다.
- **전처리기를 온전히 돌리지는 않습니다.** 리포가 정의한 매크로는 `#define`을 읽어 파일 전체에서 펼치지만,
  ...
  채택되지는 않음).
- **번들된 남의 코드는 자동으로 빼지만 완벽하진 않습니다.** 서브모듈·저작권자·커밋당 바이트로 판별하고 이유를 로그에 남깁니다(`--include-vendored`로 뒤집기). 신호가 하나도 없는 벤더 코드는 여전히 `--exclude`가 필요합니다.
...
tree-sitter 질의를 써서 측정 · **남의 코드 자동 배제**(서브모듈·저작권자·커밋당 바이트 — yaml-cpp 상수 158개 중 157개가
번들 코드였음) · **전부 읽기**(`--infer-all`, 기본 꺼짐) — 리포 전체를 묶음으로 나눠 100% 읽기,
보내기 전 규모 고지 · **안 돌아가는 검사 1회 고쳐쓰기**(실패 사유를 돌려주고, 고쳐 온 것도 검증 후 채택).

**다음:** 순서·근거·고칠 파일까지 정리한 작업 문서는 **[docs/next-steps.md](docs/next-steps.md)** 에 있습니다.
아래는 요약입니다.

1. **질의 성공률 올리기** — 실패 사유를 돌려주는 1회 재시도는 만들었지만 실측 복구율이 15건 중 3건이다.
   프롬프트의 예시를 늘릴지, 실패 유형별로 다른 힌트를 줄지는 `검사 고쳐쓰기` 집계를 보고 정한다.
2. **`--infer-all` 은 질의 성공률이 오른 뒤에 켠다** — 기능은 완성돼 있고 100% 커버도 확인했지만, 지금 켜면
   6개 파일만 읽을 때보다 결과가 나쁘다(검증된 규칙 3개 → 1개).
3. **훑기가 보여주는 것을 넓히기** — 구조 요약이 담는 것과 내장 지름길이 같은 열거라, 훑기가 짚는 곳은
   항상 지름길로 덮인다. 수식어·네임스페이스 배치 같은 축이 요약에 없으면 모델이 그쪽을 볼 이유가 없다.
4. **나머지 facet:other의 리뷰 검사** — `header_directive`(예: `#pragma once`)는 학습·검증은 되지만 리뷰
   적용이 아직 LLM 판단(참고용)이다. 다른 구조 check처럼 리뷰측 결정적 체커를 붙이면 됨.
5. **헤더 분석** — 임시 TU로 헤더를 include해 분석. 헤더 온리 프로젝트에서 clang-tidy 축이 영구 0건인 문제.
6. **비C++ 동반 파일** — *"동작이 바뀌었는데 스펙/테스트 파일이 안 바뀌었다"* 가 높은 가치의 지적입니다.
7. **출처 라벨 정정** — `reconcile`이 규칙별 판정 모델을 남기지 않아, 추론 규칙이 실제로는 강모델이 판정했는데
   learn 모델로 기록되는 작은 부정확이 있습니다.
8. 대형 diff 청킹, 결과가 유의미하면 GitHub PR 연동(App/Action).
