...
tree-sitter 질의를 써서 측정 · **전부 읽기**(`--infer-all`, 기본 꺼짐) — 리포 전체를 묶음으로 나눠 100% 읽기,
...
1. **벤더 코드 배제** — 폴더 이름 목록(`third_party` 등)이 `src/contrib` 같은 곳을 못 거른다. 전부 읽기
   실행에서 관찰의 상당수가 번들된 남의 코드 얘기였다. `.gitmodules`·git 이력처럼 리포가 스스로 말해
   주는 신호로 바꾸면 추측이 사라진다.
2. **질의 성공률 올리기** — 실패 사유를 돌려주는 1회 재시도는 만들었지만 실측 복구율이 15건 중 3건이다.
   프롬프트의 예시를 늘릴지, 실패 유형별로 다른 힌트를 줄지는 `검사 고쳐쓰기` 집계를 보고 정한다.
3. **`--infer-all` 은 위 둘이 된 뒤에 켠다** — 기능은 완성돼 있고 100% 커버도 확인했지만, 지금 켜면
   6개 파일만 읽을 때보다 결과가 나쁘다(검증된 규칙 3개 → 1개).
4. **훑기가 보여주는 것을 넓히기** — 구조 요약이 담는 것과 내장 지름길이 같은 열거라, 훑기가 짚는 곳은
   항상 지름길로 덮인다. 수식어·네임스페이스 배치 같은 축이 요약에 없으면 모델이 그쪽을 볼 이유가 없다.
5. **나머지 facet:other의 리뷰 검사** — `header_directive`(예: `#pragma once`)는 학습·검증은 되지만 리뷰
   적용이 아직 LLM 판단(참고용)이다. 다른 구조 check처럼 리뷰측 결정적 체커를 붙이면 됨.
6. **헤더 분석** — 임시 TU로 헤더를 include해 분석. 헤더 온리 프로젝트에서 clang-tidy 축이 영구 0건인 문제.
7. **비C++ 동반 파일** — *"동작이 바뀌었는데 스펙/테스트 파일이 안 바뀌었다"* 가 높은 가치의 지적입니다.
8. **출처 라벨 정정** — `reconcile`이 규칙별 판정 모델을 남기지 않아, 추론 규칙이 실제로는 강모델이 판정했는데
   learn 모델로 기록되는 작은 부정확이 있습니다.
9. 대형 diff 청킹, 결과가 유의미하면 GitHub PR 연동(App/Action).
