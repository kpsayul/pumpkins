...
- **전처리기는 돌리지 않습니다.** 매크로가 낀 class 헤더는 리포의 `#define`을 읽어 복구하지만, 그래도 못
  읽는 문법이 남습니다. 다만 **조용히 넘어가지 않습니다** — 판정 못 한 class 선언 수가 learn 출력과
...
파일만 정독 · **매크로 class 헤더 복구 + 파싱 건강상태 측정** — 리포의 `#define`을 읽어 `class FOO_API Bar`를
제대로 읽고(판정불가 25 → 0, 멤버 통계 +35%), 그래도 못 읽은 자리는 세어서 보고.
