...
1. **모델이 질의를 더 잘 쓰게 하기** — 기계는 준비됐지만 실제 실행에서 `query` 사용률이 낮고, 쓸 때도
   ...
   보고되므로, 한 번의 재시도(실패 사유를 돌려주고 다시 쓰게)가 값어치가 있는지 판단할 근거가 쌓인다.
2. **훑기가 보여주는 것을 넓히기** — 구조 요약이 담는 것과 내장 지름길이 같은 열거라, 훑기가 짚는 곳은
   항상 지름길로 덮인다. 수식어·네임스페이스 배치 같은 축이 요약에 없으면 모델이 그쪽을 볼 이유가 없다.
3. **나머지 facet:other의 리뷰 검사** — `header_directive`(예: `#pragma once`)는 학습·검증은 되지만 리뷰
   적용이 아직 LLM 판단(참고용)이다. 다른 구조 check처럼 리뷰측 결정적 체커를 붙이면 됨.
4. **헤더 분석** — 임시 TU로 헤더를 include해 분석. 헤더 온리 프로젝트에서 clang-tidy 축이 영구 0건인 문제.
5. **비C++ 동반 파일** — *"동작이 바뀌었는데 스펙/테스트 파일이 안 바뀌었다"* 가 높은 가치의 지적입니다.
6. **출처 라벨 정정** — `reconcile`이 규칙별 판정 모델을 남기지 않아, 추론 규칙이 실제로는 강모델이 판정했는데
